...
export default function UsersHeader({ onAddUser }: { onAddUser: () => void }) {
  const { t } = useTranslation();
  return (
    <header className="mb-7 flex flex-wrap-reverse items-center justify-between gap-4">
      <div className="text-start">
        <h1 className="text-3xl font-extrabold text-[#75262d]">{t("modules.users.title")}</h1>
        <p className="mt-1 text-sm font-medium text-gray-500">
          {t("modules.users.subtitle")}
        </p>
      </div>
