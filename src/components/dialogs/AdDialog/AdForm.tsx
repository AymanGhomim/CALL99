...
function NameField({ value, onChange, readOnly, error }) {
  return (
    <div>
      <FieldLabel>اسم الاعلان</FieldLabel>
      <div className="flex items-center gap-3">
        <IconSlot icon={Megaphone} />
        <input
          type="text"
          value={value}
          disabled={readOnly}
          onChange={(event) => onChange(event.target.value)}
          placeholder="مثال: خصم تبديل البطاريه الفوري"
...
function ServiceField({ value, onChange, readOnly, options, error }) {
  return (
    <div>
      <FieldLabel>الخدمه المرتبطه</FieldLabel>
      <div className="flex items-center gap-3">
        <IconSlot icon={Wrench} />
...
function DescriptionField({ value, onChange, readOnly }) {
  return (
    <div>
      <FieldLabel>وصف الاعلان</FieldLabel>
      <textarea
        rows={3}
        value={value}
        disabled={readOnly}
        onChange={(event) => onChange(event.target.value)}
        placeholder="اكتب وصفا موجزا للعرض الاعلاني............"
...
function DateRow({ form, onChange, readOnly, errors }) {
  return (
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder="مثال: 23مايو 2026"
            className={`${inputBase} px-4 text-right ${
              errors.startDate ? "border-rose-300 focus:border-rose-400" : ""
            }`}
          />
        </div>
        {errors.startDate && (
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder="مثال: 22يوليو 2026"
            className={`${inputBase} px-4 text-right ${
              errors.endDate ? "border-rose-300 focus:border-rose-400" : ""
            }`}
          />
        </div>
        {errors.endDate && (
...
/**
 * Shared fields for the create / edit / view Ad dialogs. `readOnly` renders
 * every field disabled (view mode); everything else stays a single source
 * of truth so the three dialog states can never drift visually.
 */
